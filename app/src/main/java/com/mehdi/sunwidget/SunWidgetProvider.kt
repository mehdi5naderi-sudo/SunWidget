...
class SunWidgetProvider : AppWidgetProvider() {
    companion object {
        ...
        private fun updateOne(context: Context, manager: AppWidgetManager, widgetId: Int) {
            ...
            val launch = PendingIntent.getActivity(context, widgetId,
            ...
            executor.execute {
                ...
                if (data != null) {
                    ...
                    updated.setTextViewText(R.id.status_text, "به وقت تهران • به‌روزرسانی هر ۳۰ دقیقه")
        ...
        private fun fetchSun(context: Context): SunData? {
            ...
            val today = ZonedDateTime.now(tehranZone).toLocalDate().toString()
            try {
                ...
                // formatted=0 returns UTC timestamps; explicitly convert to Tehran time.
                val sunrise = formatTehranTime(result.getString("sunrise"))
                val sunset = formatTehranTime(result.getString("sunset"))
        ...
        private fun formatTehranTime(utcTimestamp: String): String =
            Instant.parse(utcTimestamp).atZone(tehranZone).format(timeFormatter)
        ...
        private fun drawSky(sunrise: String, sunset: String): Bitmap {
            ...
            val sunriseMin = toMinutes(sunrise); val sunsetMin = toMinutes(sunset)
            ...
            val path = Path().apply { addArc(RectF(80f,horizonY-430f,w-80f,horizonY+430f),200f,140f) }
            canvas.drawPath(path,Paint().apply { color=Color.argb(110,255,255,255); style=Paint.Style.STROKE; strokeWidth=5f; isAntiAlias=true })

            // Empty at sunrise, full at noon, empty again at sunset.
            val progress = when {
                nowMin <= sunriseMin || nowMin >= sunsetMin -> 0f
                else -> {
                    val noon = sunriseMin + (sunsetMin - sunriseMin) / 2f
                    if (nowMin <= noon) ((nowMin-sunriseMin)/(noon-sunriseMin)).coerceIn(0f,1f)
                    else ((sunsetMin-nowMin)/(sunsetMin-noon)).coerceIn(0f,1f)
                }
            }
            val dayProgress = if (sunsetMin > sunriseMin) ((nowMin-sunriseMin).toFloat()/(sunsetMin-sunriseMin)).coerceIn(0f,1f) else 0.5f
            val angle = Math.PI * (1.0-dayProgress)
            val cx=w/2f; val rx=w*0.42f; val ry=h*1.02f
            val sx=cx+rx*cos(angle).toFloat(); val sy=horizonY-ry*sin(angle).toFloat()
            if (day) drawProgressSun(canvas,sx,sy,31f,progress) else {
                val p=Paint(Paint.ANTI_ALIAS_FLAG).apply { color=Color.rgb(235,241,255) }
                canvas.drawCircle(sx,sy,22f,p); p.color=Color.rgb(19,28,58); canvas.drawCircle(sx+8f,sy-5f,18f,p)
        ...
        private fun drawProgressSun(canvas: Canvas,cx: Float,cy: Float,radius: Float,progress: Float) {
            val outline=Paint(Paint.ANTI_ALIAS_FLAG).apply { color=Color.argb(220,255,226,105); style=Paint.Style.STROKE; strokeWidth=4f }
            canvas.drawCircle(cx,cy,radius,outline)
            val save=canvas.save(); val clip=Path().apply { addCircle(cx,cy,radius-2f,Path.Direction.CW) }; canvas.clipPath(clip)
            val fillTop=cy+radius*2f*(1f-progress)
            val fill=Paint(Paint.ANTI_ALIAS_FLAG).apply { shader=LinearGradient(0f,fillTop,0f,cy+radius,Color.rgb(255,237,116),Color.rgb(255,171,45),Shader.TileMode.CLAMP) }
            canvas.drawRect(cx-radius,fillTop,cx+radius,cy+radius,fill); canvas.restoreToCount(save)
            if (progress>0.05f) {
                val glow=Paint(Paint.ANTI_ALIAS_FLAG).apply { color=Color.argb((55+45*progress).toInt(),255,220,80) }
                canvas.drawCircle(cx,cy,radius+17f,glow)
            }
        }

        private fun toMinutes(value: String): Int { val p=value.split(":"); return p[0].toInt()*60+p[1].toInt() }
    }

    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) { appWidgetIds.forEach { updateOne(context,appWidgetManager,it) } }
